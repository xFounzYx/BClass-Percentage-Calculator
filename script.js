const translations = {
    ru: {
        ...
        redirectbutton: "Перейти в BilimClass.kz",
		...
		decimalPlacesLabel: "Округлить до",
		zero: "0 знаков",
		one: "1 знака",
		two: "2 знаков",
		three: "3 знаков",
    ...
    en: {
        ...
        redirectbutton: "Go to BilimClass.kz",
        ...
        quarter: "Quarter",
        halfYear: "Half-year",
		decimalPlacesLabel: "Round up to",
		zero: "0 characters",
		one: "1 character",
		two: "2 characters",
		three: "3 characters",
    ...
    kk: {
        ...
        redirectbutton: "BilimClass.kz өту",
		...
		decimalPlacesLabel: "Дейін дөңгелектеңіз",
		zero: "0 таңба",
		one: "1 таңба",
		two: "2 таңба",
		three: "3 таңба",
...
let decimalPlaces = 1; // По умолчанию округляем до 1 знака после запятой
...
function changeLanguage() {
	...
	document.getElementById('decimalPlaces-label').textContent = strings.decimalPlacesLabel;
	document.getElementById('zero').textContent = strings.zero;
	document.getElementById('one').textContent = strings.one;
	document.getElementById('two').textContent = strings.two;
	document.getElementById('three').textContent = strings.three;

    // Добавляем крестик для закрытия модального окна
    const modalContent = document.querySelector('.modal-content');
    if (!modalContent.querySelector('.modal-close')) {
        modalContent.innerHTML = `<span class="modal-close" onclick="closeInstruction()">&times;</span>` + modalContent.innerHTML;
    }
...
function updateResult({ subject, percentageFoSor, percentageSoch, overallPercentage, grade }) {
    ...
    let resultDiv = `
        ...
        ${strings.resultOverall}: ${formatPercentage(percentageFoSor)}%
    `;
...
window.onload = function() {
    ...
    if (foScores) document.getElementById("foScores").value = foScores;
    if (sorScores) document.getElementById("sorScores").value = sorScores;
...
// Добавляем возможность выбора количества знаков после запятой
